...
class JagannathSemanticTokensProvider {
    ...
    tokenizeLine(builder, line, lineNum) {
        // Divine Weapons (Astras) - Optimization passes
        this.matchPattern(builder, line, lineNum, /\b(Brahmastra|Pashupatastra|Agneyastra|Varunastra|SudarshanaChakra|Narayanastra|Vayavyastra|Nagastra|Garudastra|Mohiniastra|Vajra|Trishula|Gandiva|Vijaya)\b/g, "astra", ["definition"]);
        // Narakas - Error types from Garuda Purana
        this.matchPattern(builder, line, lineNum, /\b(Tamisram|Andhakupa|Raurava|Suchimukha|Vaitarani|Kalasutra|Asipatravana|Kumbhipaka|Preta|Pranarodha|Visasana)\b/g, "naraka", ["deprecated"]);
        // Guṇas - Quality levels
        this.matchPattern(builder, line, lineNum, /\b(sattva|rajas|tamas|sāttvika|rājasika|tāmasika)\b/g, "guna", []);
        // Kośas - Memory sheaths
        this.matchPattern(builder, line, lineNum, /\b(anna(maya)?|prāṇa(maya)?|mano(maya)?|vijñāna(maya)?|ānanda(maya)?)\b/gi, "kosha", []);
        // Chakras - Software layers
        this.matchPattern(builder, line, lineNum, /\b(mūlādhāra|muladhara|svādhiṣṭhāna|svadhisthana|maṇipūra|manipura|anāhata|anahata|viśuddha|vishuddha|ājñā|ajna|sahasrāra|sahasrara)\b/g, "chakra", []);
        // Varṇas - Privilege rings
        this.matchPattern(builder, line, lineNum, /\b(brāhmaṇa|brahmana|brahmin|kṣatriya|kshatriya|vaiśya|vaishya|śūdra|shudra)\b/g, "varna", []);
        // Mārgas - Optimization paths
        this.matchPattern(builder, line, lineNum, /\b(karma(yoga)?|jñāna(yoga)?|jnana(yoga)?|bhakti(yoga)?|rāja(yoga)?|raja(yoga)?)\b/g, "marga", []);
        // Kārakas - Semantic roles
        this.matchPattern(builder, line, lineNum, /\[(kartṛ|kartr|karman|karaṇa|karana|sampradāna|sampradana|apādāna|apadana|adhikaraṇa|adhikarana|sambandha|hetu)\]/g, "karaka", []);
        // Mantras - Sacred invocations
        this.matchPattern(builder, line, lineNum, /\b(oṁ|om|namaḥ|svāhā|phaṭ|huṁ)\b/g, "mantra", []);
        // Affixes - Type modifiers
        this.matchPattern(builder, line, lineNum, /-[aāblgkhpsr](?=[-\s\{\(]|$)/g, "modifier", ["declaration"]);
        // Thread-safety and security affixes
        this.matchPattern(builder, line, lineNum, /-(sūtra|sutra|vaitaraṇī|vaitarani)\b/g, "modifier", ["threadsafe"]);
        // Lifetime annotations
        this.matchPattern(builder, line, lineNum, /\^[0-9]+|'[a-z][a-z0-9_]*/g, "lifetime", []);
        // Macros
        this.matchPattern(builder, line, lineNum, /\b(mudraṇa|mudrana|oṁ|om|parikṣā|pariksha|debug|assert|panic|todo|unreachable)!/g, "macro", []);
    }
    matchPattern(builder, line, lineNum, pattern, tokenType, modifiers) {
        let match;
        while ((match = pattern.exec(line)) !== null) {
            ...
            if (typeIndex >= 0) {
                ...
                builder.push(lineNum, match.index, match[0].length, typeIndex, modifierBits);
            }
        }
    }
}
...
// =============================================================================
class JagannathCodeLensProvider {
...
class JagannathHoverProvider {
    ...
    provideHover(document, position) {
        const range = document.getWordRangeAtPosition(position, /[a-zA-Zā-ṛĀ-Ṛ_][a-zA-Zā-ṛĀ-Ṛ0-9_-]*/);
        if (!range)
            return null;
        const word = document.getText(range);
        // Check glossary
        const entry = this.glossary.get(word) || this.glossary.get(word.toLowerCase());
        if (entry) {
...
class ASTViewerPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel("jagannathAstViewer", "🕉️ AST Viewer - Jagannath", column || vscode.ViewColumn.Two, {
            enableScripts: true,
...
class PipelineViewerPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel("jagannathPipeline", "🔄 Compilation Pipeline - Sāṃkhya", column, { enableScripts: true, retainContextWhenHidden: true });
        PipelineViewerPanel.currentPanel = new PipelineViewerPanel(panel, extensionUri);
    }
...
function activate(context) {
    ...
    const clientOptions = {
        ...
        synchronize: {
            fileEvents: vscode.workspace.createFileSystemWatcher("**/*.{jag,jagannath}"),
        },
    };
    // Create and start LSP client
    client = new node_1.LanguageClient("jagannathLanguageServer", "Jagannath Language Server", serverOptions, clientOptions);
    // Register semantic tokens provider
    context.subscriptions.push(vscode.languages.registerDocumentSemanticTokensProvider({ language: "jagannath" }, new JagannathSemanticTokensProvider(), legend));
    // Register code lens provider
    context.subscriptions.push(vscode.languages.registerCodeLensProvider({ language: "jagannath" }, new JagannathCodeLensProvider()));
    // Register hover provider
    context.subscriptions.push(vscode.languages.registerHoverProvider({ language: "jagannath" }, new JagannathHoverProvider()));
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand("jagannath.build", buildCommand), vscode.commands.registerCommand("jagannath.run", runCommand), vscode.commands.registerCommand("jagannath.format", formatCommand), vscode.commands.registerCommand("jagannath.invokeAstra", invokeAstraCommand), vscode.commands.registerCommand("jagannath.yamaJudge", yamaJudgeCommand), vscode.commands.registerCommand("jagannath.selectMarga", selectMargaCommand), vscode.commands.registerCommand("jagannath.showAstViewer", () => {
    ...
    }), vscode.commands.registerCommand("jagannath.analyzeFunction", (name, uri, line) => {
        vscode.window.showInformationMessage(`🕉️ Analyzing function: ${name}`);
        ASTViewerPanel.createOrShow(context.extensionUri);
    }), vscode.commands.registerCommand("jagannath.viewMemoryLayout", (name, uri, line) => {
        vscode.window.showInformationMessage(`📊 Memory layout for: ${name}`);
    }), vscode.commands.registerCommand("jagannath.viewAstraOptimization", (uri, line) => {
        vscode.window.showInformationMessage(`⚔️ Astra optimization analysis at line ${line + 1}`);
    }), vscode.commands.registerCommand("jagannath.viewNarakaClassification", (uri, line) => {
        vscode.window.showInformationMessage(`🔥 Naraka error classification at line ${line + 1}`);
    }));
    // Register diagnostics provider (simple Naraka mapper)
    const diagnosticCollection = vscode.languages.createDiagnosticCollection("jagannath-naraka");
    context.subscriptions.push(diagnosticCollection);
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc) => {
        if (doc.languageId !== "jagannath")
            return;
        const diagnostics = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const p of patterns) {
                ...
                if (m) {
                    ...
                    const diag = new vscode.Diagnostic(range, `${p.message} — Naraka: ${p.naraka}. Penance: apply śuddhi-kri()`, p.severity);
                    diag.source = "Yama";
                    diag.code = p.naraka;
...
async function buildCommand() {
    ...
    if (editor && editor.document.languageId === "jagannath") {
        ...
        const config = vscode.workspace.getConfiguration("jagannath");
        const guna = config.get("guna", "rajas");
...
async function invokeAstraCommand() {
    ...
    vscode.window.showInformationMessage(`Invoking ${astra} ${mantra ? `with mantra: ${mantra}` : "with default mantra"}`);
}
async function yamaJudgeCommand() {
    ...
    vscode.window.showInformationMessage("Yama Judge: Static analysis completed — check Problems panel for Naraka diagnostics");
}
...
function registerNarakaProviders(context) {
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider({ language: "jagannath" }, new NarakaCodeActionProvider(), { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }));
    context.subscriptions.push(vscode.commands.registerCommand("jagannath.applyPenance", async (uri, range, naraka) => {
        const doc = await vscode.workspace.openTextDocument(uri);
        const editor = await vscode.window.showTextDocument(doc);
        // Simple penance insertion: add a comment with recommended fix
        await editor.edit((edit) => {
            edit.insert(new vscode.Position(range.end.line + 1, 0), `// PRĀYAŚCITTA: apply śuddhi-kri() to resolve ${naraka}\n`);
        });
